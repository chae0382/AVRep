...
class RandomMaskingGenerator2D:
    def __init__(self, input_size, mask_ratio):
        ...
        self.num_patches_per_t = self.width
        self.num_masks_per_t = int(mask_ratio * self.width)
    ...
    def __call__(self,vis):
        vis_mask = np.hstack([
            np.zeros(vis*(self.num_patches_per_t - self.num_masks_per_t)),  # 0: for unmasked
            np.ones(vis*self.num_masks_per_t),  # 1: for masked
        ])
        np.random.shuffle(vis_mask)
        invis = self.height - vis
        invis_mask = np.hstack([
            np.zeros(invis * (self.num_patches_per_t - self.num_masks_per_t)),  # 0: for unmasked
            np.ones(invis * self.num_masks_per_t),  # 1: for masked
        ])
        np.random.shuffle(invis_mask)
        mask = np.hstack([
            vis_mask, invis_mask
        ])
        return mask
...
class TubeMaskingGenerator:
    ...
    def __call__(self, frames):
        self.frames = frames
        self.total_patches = frames * self.num_patches_per_frame
        self.total_masks = frames * self.num_masks_per_frame
...
class MeshMaskingGenerator:
    def __init__(self, input_size, mask_ratio):
        self.frames, self.vertices = input_size
        self.total_patches = self.frames
        self.total_masks = int(self.frames * mask_ratio)
    ...
    def __call__(self, frames):
        mask = np.hstack([
            np.zeros(self.frames - self.total_masks),
