...
from datasets_av_cy import build_pretraining_dataset
from engine_for_pretraining_av_cy_syncnet import train_one_epoch, val_one_epoch
from utils import NativeScalerWithGradNormCount as NativeScaler
import utils
import modeling_pretrain_av_cy
...
def get_args():
    ...
    parser.add_argument('--data_path', default='/path/to/list_kinetics-400', type=str,
                        help='dataset path')
    parser.add_argument('--data_root', default='/path/to/list_kinetics-400', type=str,
    ...
    parser.add_argument('--no_augmentation', action='store_true',
                        help="do not use GroupMultiScaleCrop during pre-training")
    ...
    parser.add_argument('--decoder_depth_audio', default=4, type=int,
                        help='depth of audio decoder')
    parser.add_argument('--mask_type_audio', default='random', choices=['random'],
    ...
    # for diff target
    parser.add_argument('--use_frame_diff_as_target', action='store_true',
                        help="use frame difference as partial recontruction targets")
    parser.add_argument('--frame_diff_group_size', default=2, type=int,
                        help="the group size for diff calculation, for example, "
                             "the target is [f0, f1-f0, f2-f0, f3-f0], [f4, f5-f4, f6-f4, f7-f4], ..., when the size is 4.")
    parser.add_argument('--target_diff_weight', default=None, type=float,
                        help="the weight (0-1) for frame diff target ")
...
def get_model(args):
    ...
    model = create_model(
        ...
        encoder_depth_audio=args.encoder_depth_audio,
        decoder_depth_audio=args.decoder_depth_audio,
        encoder_fusion_depth=args.encoder_fusion_depth,
        inter_contrastive_temperature=args.inter_contrastive_temperature,
        return_intermediate_features=args.return_intermediate_features,
    )
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #     optimizer, device, epoch, loss_scaler,
        #     args.clip_grad, log_writer=None,
        #     start_steps=epoch * num_training_steps_per_epoch,
        #     lr_schedule_values=lr_schedule_values,
        #     wd_schedule_values=wd_schedule_values,
        #     patch_size=patch_size[0],
        ...
        #     target_diff_weight=args.target_diff_weight,
        # )
        # val_result = {'val_loss':val_loss, 'val_loss_mae':val_loss_mae, 'val_loss_mae_a':val_loss_mae_a,
        #               'val_loss_mae_v':val_loss_mae_v, 'val_loss_contrastive':val_loss_contrastive, 'val_loss_contrastive_a':val_loss_contrastive_a, 'val_loss_contrastive_v':val_loss_contrastive_v}
        # val_result = {k:float(v) for k,v in val_result.items()}
        # if args.output_dir and utils.is_main_process():
        #     with open(os.path.join(args.output_dir, "val_log.txt"), mode="a", encoding="utf-8") as f:
        #         f.write(json.dumps(val_result) + "\n")
        #
        # if val_loss < best_loss:
        #     best_loss = val_loss
        #     best_epoch = epoch
        # print(f'Set best epoch {best_epoch}')
