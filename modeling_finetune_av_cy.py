...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, num_frames=12, tubelet_size=2):
        super().__init__()
        ...
        self.input_token_size = (
        num_frames // self.tubelet_size, img_size[0] // patch_size[0], img_size[1] // patch_size[1])
...
class PatchEmbedMesh(nn.Module):
    """ Flexible Image to Patch Embedding
    """
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 # do not perform temporal pooling, has higher priority than 'use_mean_pooling'
...
class AudioVisionTransformer(nn.Module):
    ...
    def forward_features(self, x, x_audio):
        ...
        video_feature = torch.sum(
            video_feature * torch.nn.functional.softmax(self.video_layer_weights, dim=0).view(1, -1, 1), dim=1)
        audio_feature = torch.sum(
            audio_feature * torch.nn.functional.softmax(self.audio_layer_weights, dim=0).view(1, -1, 1), dim=1)
...
class RetrievalAudioVisionTransformer(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def __init__(self,
        ...
        self.encoder_depth = depth
        if depth > 0:
            self.encoder = RetrievalVisionTransformerEncoder(
                img_size=img_size,
                patch_size=patch_size,
                in_chans=in_chans,
                num_classes=0,  # no head for video encoder
        ...
        self.encoder_depth_audio = depth_audio
        if depth_audio > 0:
            self.encoder_audio = RetrievalVisionTransformerEncoder2D(
                img_size=img_size_audio,
                patch_size=patch_size_audio,
                in_chans=in_chans_audio,
                num_classes=0,  # no head for audio encoder
...
class RetrievalAudioVisionTransformerMesh(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def __init__(self,
        ...
        self.encoder_depth = depth
        # video encoder
        self.encoder = RetrievalVisionTransformerEncoderMesh(
...
class RetrievalConformerEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        super(RetrievalConformerEncoder, self).__init__()
    ...
    def forward(self, xs, masks=None,return_intermediate_features=None):
...
class RetrievalAudioVisionTransformerMeshConformer(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def __init__(self,
        ...
        # audio encoder
        self.encoder_audio = RetrievalConformerEncoder()
...
class ClassificationVisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        fc_num = embed_dim
        self.head = nn.Linear(fc_num, num_classes) if num_classes > 0 else nn.Identity()
        self.head_activation_func = torch.nn.Softmax(dim=1)
    ...
    def forward(self, x):
        ...
        # unimodal feature
        video_feature = x.mean(dim=1)  # [B, L, C]
        logits = self.head(video_feature)
        return logits


class RetrievalVisionTransformerEncoder(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, num_frames=max_length, tubelet_size=tubelet_size)
        num_patches = self.patch_embed.num_patches
...
class RetrievalVisionTransformerEncoderMesh(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
...
class RetrievalVisionTransformerEncoder2D(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
...
@register_model
def retrieval_dim512_patch16_160_a64(pretrained=False, **kwargs):
    embed_dim = 512
    num_heads = 8
    patch_size = 16
    model = RetrievalAudioVisionTransformer(
        # video
        img_size=160,
        patch_size=patch_size, embed_dim=embed_dim, num_heads=num_heads, mlp_ratio=4, qkv_bias=True,
        # audio
        img_size_audio=(64, 128),  # (T, F)
        patch_size_audio=patch_size, embed_dim_audio=embed_dim, num_heads_audio=num_heads,norm_layer=partial(nn.LayerNorm, eps=1e-6),max_length=16, **kwargs)
    model.default_cfg = _cfg()
    return model

@register_model
def retrieval_dim512_patch16_160_a64_mesh(pretrained=False, **kwargs):
    model = RetrievalAudioVisionTransformerMesh(
        vertex_size=254 * 3,
        patch_size=254 * 3,
        embed_dim=512,
        num_heads=8,
...
@register_model
def retrieval_dim512_patch16_160_a64_full_mesh(pretrained=False, **kwargs):
    model = RetrievalAudioVisionTransformerMesh(
        vertex_size=5023 * 3,
        patch_size=5023 * 3,
        embed_dim=512,
...
@register_model
def retrieval_dim512_patch16_160_a64_full_mesh_conformer(pretrained=False, **kwargs):
    model = RetrievalAudioVisionTransformerMeshConformer(
        vertex_size=5023 * 3,
        patch_size=5023 * 3,
        embed_dim=768,
        num_heads=12,
...
@register_model
def classification_dim512_patch16_160_a64(pretrained=False, **kwargs):
    embed_dim = 512
    num_heads = 8
    patch_size = 16
